...
(function (root) {
  ...
  const TOPICS = [
    { t: 'Editing individual islands', k: 'edit transform island chart rotate scale translate move', b: 'Click a UV island or chart in 3D, then use the Selected chart controls to rotate, scale or move it. Positive scale and finite inputs are required. Undo and redo restore exact coordinates. Edits may introduce overlaps or move outside 0–1; inspect the diagnostics. Repack establishes padding again and chooses new positions and orientations. Disable Equalise texel density to retain relative island sizes.' },
    { t: 'Mesh health preflight', k: 'mesh health duplicate degenerate collapsed sliver non manifold winding topology repair', b: 'The Mesh health card checks duplicates, collapsed faces, very thin triangles, non-manifold edges and inconsistent winding. Open boundaries are supported. Diagnostics suggest mesh-editor repairs; they do not modify geometry or certify that surfaces have no 3D intersections.' },
    { t: 'Portable project files', k: 'project save load restore textures materials version', b: 'Save project embeds source textures, material colours, transforms, face assignments, mesh, source UVs, current layout, seams and settings. Baked images are regenerated after reopening. Old v1 files load with a warning if materials were not saved. Invalid or cancelled loads preserve existing work.' },
    ...
    { t: 'Imported UVs', k: 'imported original existing uv grade analyse seams source', b: 'If the file already has UVs, grade them with "Grade imported UVs". "Use imported layout" preserves their exact coordinates and internal seams so you can edit, relax or repack them. "Use imported seams" instead re-flattens the mesh using those seams. Return to current result leaves an existing bake unchanged.' },
    { t: 'Packing', k: 'pack packing bitmap skyline padding rotation texture size resolution', b: 'Bitmap packing rasterises every chart and nests shapes into each other\'s gaps (xatlas / Blender style), usually 10–25% denser than rectangles. Padding is in texels at the chosen texture size: an effective margin of p texels gives an estimated mip allowance of floor(log2(p)), depending on target filtering. Requested margins may be reduced if charts cannot fit; imported or manually edited layouts have unknown margins until repacked. Rotations let charts turn in 90° steps. Repack (P) re-runs packing only.' },
    { t: 'Texel density', k: 'texel density px unit even equalise cv', b: 'Texel density is how many texels cover one model unit. "Equalise texel density" scales every chart so detail is even across the model; Density CV measures variation across triangles, including within each island (0% is perfect). Reports retain chartCV for island-average variation.' },
